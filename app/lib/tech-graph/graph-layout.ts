...
function detect_group_membership(nodes: GraphNode[]): Map<string, string> {
    ...
    for (const node of nodes) {
        if (!node.research_family) continue;
        const members = groups.get(node.research_family) ?? [];
        members.push(node);
        groups.set(node.research_family, members);
    }
    const result = new Map<string, string>();
    for (const [family, members] of groups) {
        if (members.length <= 2) continue;
        for (const node of members) result.set(node.id, family);
    }
    return result;
}
